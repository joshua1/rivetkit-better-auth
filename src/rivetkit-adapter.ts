...
import { createAdapter, CustomAdapter, type AdapterDebugLogs } from "better-auth/adapters"
...
export const rivetKitAdapter = (actorClient: ActorServerClient, config: RivetKitAdapterConfig = {}) =>
	createAdapter({
		config: {
		...
		adapter: ({ debugLog }) => {
			...
			return {
				...
				create: async <T extends Record<string, any>>({ data, model, select }: Parameters<CustomAdapter['create']>[0]) => {
				...
				findOne: async<T>({ model, where, select }: Parameters<CustomAdapter["findOne"]>[0]) => {
				...
				findMany: async <T>({ model, where, limit, sortBy, offset }: Parameters<CustomAdapter['findMany']>[0]) => {
				...
				updateMany: async ({ model, where, update }: Parameters<CustomAdapter['updateMany']>[0]) => {
