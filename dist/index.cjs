...
__export(index_exports, {
  RivetKitBetterAuthError: () => RivetKitBetterAuthError,
  createLinqPredicate: () => createLinqPredicate,
  defaultActions: () => defaultActions,
  defaultActorState: () => defaultActorState,
  rivetKitAdapter: () => rivetKitAdapter,
...
var RivetKitBetterAuthError = class extends import_better_auth.BetterAuthError {
...
var import_linq_extensions = require("linq-extensions");
...
// src/rivetkit-adapter.ts
var rivetKitAdapter = (actorClient, config = {}) => (0, import_adapters.createAdapter)({
  config: {
  ...
  adapter: ({ debugLog }) => {
    ...
    return {
      ...
      findOne: async ({ model, where, select }) => {
        debugLog("findOne", `[RivetKit Adapter] Finding one ${model}:`, where);
        console.log("findOne", { model, where, select });
        try {
          const result = await authActor.findOne({
            model,
            where: createLinqPredicate(where),
            select
          });
          console.log("findOne", `[RivetKit Adapter] Found ${model}:`, result);
          return result;
        } catch (error) {
      ...
      findMany: async ({ model, where, limit, sortBy, offset }) => {
        debugLog("findMany", `[RivetKit Adapter] Finding many ${model}:`, { where, limit, sortBy, offset });
        try {
          ...
          debugLog("findMany", `[RivetKit Adapter] Found ${model} records:`, result?.length);
          return result;
        } catch (error) {
          console.error(`[RivetKit Adapter] Error finding many ${model}:`, error);
          throw error;
        }
      },
      update: async ({ model, where, update }) => {
        debugLog("update", `[RivetKit Adapter] Updating ${model}:`, { where, update });
        try {
      ...
      updateMany: async ({ model, where, update }) => {
        debugLog("updateMany", `[RivetKit Adapter] Updating many ${model}:`, { where, update });
        try {
          ...
          debugLog("updateMany", `[RivetKit Adapter] Updated ${model} records:`, result);
          return result;
        } catch (error) {
          console.error(`[RivetKit Adapter] Error updating many ${model}:`, error);
          throw error;
        }
      },
      ...
      deleteMany: async ({ model, where }) => {
        debugLog("deleteMany", `[RivetKit Adapter] Deleting many ${model}:`, where);
        try {
          const result = await authActor.deleteMany({
            model,
            where: createLinqPredicate(where)
          });
          debugLog("deleteMany", `[RivetKit Adapter] Deleted ${model} records:`, result);
          return result;
        } catch (error) {
          console.error(`[RivetKit Adapter] Error deleting many ${model}:`, error);
          throw error;
        }
      },
...
var import_linq_extensions2 = require("linq-extensions");
var defaultActions = () => {
  return {
    // Create operation
    create: async (c, params) => {
      try {
        ...
        if (c.state[tableName]) {
          const recordExists = c.state[tableName]?.any((k) => k.id === params.data.id);
          if (recordExists) {
    ...
    findOne: async (c, params) => {
      try {
        const tableName = c.vars.tableNames[params.model];
        if (c.state[tableName]) {
          return c.state[tableName]?.firstOrNull(params.where);
        }
        throw new RivetKitBetterAuthError(`Table not found in state: ${tableName}`);
      } catch (error) {
    ...
    updateMany: async (c, params) => {
      try {
        const tableName = c.vars.tableNames[params.model];
        if (c.state[tableName]) {
          const records = c.state[tableName]?.where(params.where).map((item) => ({ ...item, ...params.update })).toArray();
